    ...
    <script type="text/javascript">
        ...
        async function run(publicKey){
            let result =await navigator.credentials.get({publicKey}).catch ((err) => {
                $("#error").val(err);
                $("#webauth").submit();
            });
    ...
    <input type="submit" class="${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!} ${properties.kcButtonBlockClass!} ${properties.kcButtonLargeClass!}" id="registerWebAuthnAIA" value="${msg("doLogIn")}" onclick="doWebauthn()" ontouch="doWebauthn" autofocus />
    <!-- <a href="#" id="try-another-way" onclick='$("#error").val("canceled");$("#webauth").submit();'>${msg("doTryAnotherWay")}</a> -->
