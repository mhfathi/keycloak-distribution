    ...
    <#elseif section = "form">
        ...
        <script type="text/javascript">
            ...
            async function run(publicKey){
                let result =await navigator.credentials.create({publicKey}).catch ((err) => {
                    $("#error").val(err);
                    $("#register").submit();
                });
                ...
                let initLabel = "WebAuthn ".concat(publicKey.rp.name);
                let labelResult = window.prompt("Please assign a custom label to this device", initLabel);
